...
*/
package pt.haslab.dataflasks.simulation;
...
import pt.haslab.dataflasks.bootstrapper.Bootstrapper;
...
public class MainSimulation {
	...
	private static String wikifolderpath;
	private static int nrwikisnapshots;
	...
	public static void main(String[] args) {
		...
		try {
			...
			clienttimeout = Long.parseLong(prop.getProperty("clientTimeout"))*1000L;
			//Wikipedia Loader
			long wikiload = Long.parseLong(prop.getProperty("wikiload"));
			wikifolderpath = prop.getProperty("wikidatafolder");
			nrwikisnapshots = Integer.parseInt(prop.getProperty("nrwikisnapshots"));
			//CHURN
			...
			//PRINTING INFO
			System.out.println("times -> bootime: "+boottime+" ycsbLoad: "+initload+" ycsbRun: "+initrun + " wikiload: "+wikiload);
			...
			Entry<Bootstrapper> booter = bootproc.createEntry(pt.haslab.dataflasks.bootstrapper.Bootstrapper.class,pt.haslab.dataflasks.bootstrapper.BootstrapperImpl.class.getName());
			...
			Entry<Peer>[] e = world.createEntries(number_of_peers,pt.haslab.dataflasks.core.Peer.class,pt.haslab.dataflasks.core.PeerImpl.class.getName());
			...
			Entry<TimeAdvancer> time = timeproc.createEntry(pt.haslab.dataflasks.simulation.TimeAdvancer.class,pt.haslab.dataflasks.simulation.TimeAdvancerImpl.class.getName());
			...
			String wikiip = "";
			if(wikiload!=0){
				wikiip = queueWikiLoad(wikiload);
				System.out.println("Setting up WikiLoad: \""+"cp -r "+wikifolderpath+"* "+wikiip+"\"");
				Runtime.getRuntime().exec(new String[] {"/bin/sh", "-c", "cp -r "+wikifolderpath+"* "+wikiip});
			...
			Runtime.getRuntime().exec("echo "+firstip+" > bootip.txt ");
			if(!loadip.equals("")){
				Runtime.getRuntime().exec("cp peerlist.properties "+loadip);
				Runtime.getRuntime().exec("cp bootip.txt "+loadip);
			}
			if(!runip.equals("")){
				Runtime.getRuntime().exec("cp peerlist.properties "+runip);
				Runtime.getRuntime().exec("cp bootip.txt "+runip);
			}
...
//								}
	...
	private static String addPeer(double spos) throws ContainerException, IllegalArgumentException, SecurityException, ClassNotFoundException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		...
		Entry<Peer> newpeer = newproc.createEntry(pt.haslab.dataflasks.core.Peer.class,pt.haslab.dataflasks.core.PeerImpl.class.getName());
	...
	private static String queueYCSBRun(long runtime){
		...
		try{
			...
			ycsb.at(runtime,TimeUnit.SECONDS).queue().main("com.yahoo.ycsb.Client","-t","-s","-threads","1","-db","client.YCSBGlue","-p","exportfile=ycsbRUN.txt",
					...
					"-p","stratus.bootip="+firstip,"-p","stratus.lbinterval="+psssleepinterval,"-P", "workloads/workloadb");
	...
	private static String queueWikiLoad(long runtime){
		String wikiip = "";
		try{
			Host wikihost = world.createHost();
			Process wikiproc = wikihost.createProcess();
			Entry<Main> wiki = wikiproc.createEntry();
